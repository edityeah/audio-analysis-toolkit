...
# ── UI polish layer (additive — overrides tighter, riskier rules above) ──
st.markdown("""
<style>
/* App-wide ambient gradient backdrop — deep indigo base with purple + teal glows */
div.stApp {
    background:
        ...
        linear-gradient(180deg, #15102b 0%, #0f0b1e 50%, #0c0a17 100%) !important;
}

/* Restore the Streamlit top header so the sidebar expand arrow is reachable.
   Keep it transparent and slim so it doesn't add a visible bar. */
header[data-testid="stHeader"] {
    display: flex !important;
...
/* Make sure the collapsed-sidebar reveal button is visible & clickable */
...
[data-testid="collapsedControl"] svg {
    ...
    fill: #b5a8ff !important;
}

/* Reclaim the blank space above the title */
.block-container {
...
/* Sidebar tinted to match the new indigo palette */
section[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #1a1438 0%, #14102a 100%) !important;
    border-right: 1px solid rgba(124, 92, 255, 0.15) !important;
}

/* Let the indigo gradient show through — inner containers were
   painting solid #111215 over it, which is why the title sat in a black box. */
.block-container,
.main,
...
/* Smaller, refined sidebar logo with subtle glow */
.logo-center-container, .logo-container {
    display: flex !important;
    justify-content: center !important;
    margin: 0.25rem 0 1rem 0 !important;
}
.logo-centered, .logo-image-large, .logo-placeholder-large {
    max-width: 140px !important;
    width: 140px !important;
    height: auto !important;
    border-radius: 14px !important;
    box-shadow: 0 6px 24px rgba(124, 92, 255, 0.28) !important;
    transition: transform 0.3s ease, box-shadow 0.3s ease !important;
}
.logo-centered:hover, .logo-image-large:hover {
    transform: scale(1.04) !important;
    box-shadow: 0 8px 32px rgba(124, 92, 255, 0.42) !important;
}

/* Animated gradient title */
.main-header, h1.main-header, .stMarkdown h1.main-header {
    background: linear-gradient(90deg, #7c5cff 0%, #00d4ff 50%, #ff5ca8 100%) !important;
    background-size: 200% auto !important;
    -webkit-background-clip: text !important;
    -webkit-text-fill-color: transparent !important;
    background-clip: text !important;
    color: transparent !important;
    animation: aat-shimmer 6s ease-in-out infinite !important;
    text-shadow: none !important;
    font-weight: 800 !important;
}
@keyframes aat-shimmer {
    0%, 100% { background-position: 0% center; }
    50% { background-position: 100% center; }
}

/* Glassy welcome card with gradient border */
.welcome-card {
    background: linear-gradient(160deg, rgba(35, 39, 47, 0.55), rgba(17, 18, 21, 0.55)) !important;
...
.welcome-card h3 {
    background: linear-gradient(90deg, #ffffff, #b5a8ff) !important;
    -webkit-background-clip: text !important;
    -webkit-text-fill-color: transparent !important;
    background-clip: text !important;
    font-weight: 800 !important;
...
.bullet-section li strong {
    color: #b5a8ff !important;
}

/* Feature cards: gradient surface, hover lift, accent glow */
.metric-card {
    background: linear-gradient(160deg, #1a1d24, #111215) !important;
...
.metric-card::before {
    ...
    position: absolute;
    inset: 0;
    background: linear-gradient(135deg, rgba(124, 92, 255, 0.14), transparent 65%);
    opacity: 0;
...
.metric-card:hover {
    ...
    box-shadow: 0 14px 36px rgba(124, 92, 255, 0.25) !important;
}
...
/* Pill tabs with active gradient */
.stTabs [data-baseweb="tab-list"] {
...
/* Sidebar success / file pill warmer accent */
section[data-testid="stSidebar"] .stSuccess {
    ...
    with st.sidebar:
        ...
        if logo_path.exists():
            ...
            <div class="logo-center-container">
                <a href="https://news.adityeah.in/" target="_blank" rel="noopener" title="Visit my blog">
        ...
        st.markdown('<h2 class="sidebar-header">Provide Your Audio</h2>', unsafe_allow_html=True)

        mode = st.radio(
            "Input mode",
            ["📁 Upload a file", "🎙️ Record live"],
            horizontal=True,
        ...
        if mode == "📁 Upload a file":
            # Switching modes clears any pending recording
            st.session_state.pop("submitted_recording", None)
            ...
            st.caption("⏱️ Record up to ~30 minutes. Click the mic to start, click again to stop, then submit.")
            ...
            if recorded is not None:
                st.audio(recorded)
                size_mb = recorded.size / (1024 * 1024)
                st.caption(f"Recording size: **{size_mb:.2f} MB**")

                # Soft 30-min guard: WAV at browser default is ~10 MB/min,
                # so >320 MB is almost certainly past the limit.
                if recorded.size > 320 * 1024 * 1024:
                    st.warning("⚠️ Recording looks longer than ~30 minutes. Please re-record a shorter clip.")
                else:
                    if st.button("✅ Submit Recording", use_container_width=True, type="primary"):
                        # Give it a friendly filename so downstream code sees a .wav
                        recorded.name = "live_recording.wav"
                        st.session_state.submitted_recording = recorded

            # Use the submitted recording (persists across reruns) as the active audio
            if "submitted_recording" in st.session_state:
                audio_file = st.session_state.submitted_recording

        # Add copyright and blog link below the input controls
        st.markdown("""
        <hr style='border: 1px solid #444; margin-top: 2rem; margin-bottom: 1rem;'>

        <div style='text-align: center; font-size: 0.9rem; color: #bbb;'>
        © Adityeah | <a href='https://news.adityeah.in/' target='_blank' style='color: #4f8cff; font-weight: bold; text-decoration: none;'>Visit my Blog</a>
        </div>
        <div style='text-align: center; margin-top: 0.65rem;'>
            <a href='https://www.linkedin.com/in/adityacbcc/' target='_blank' rel='noopener' title='Connect on LinkedIn'
               style='display: inline-flex; align-items: center; justify-content: center;
                      width: 28px; height: 28px; border-radius: 6px; background: #0A66C2;
                      transition: transform 0.2s ease, box-shadow 0.2s ease;'
               onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 4px 12px rgba(10,102,194,0.5)';"
        ...
        if audio_file is not None:
            label = "Recording submitted!" if mode == "🎙️ Record live" else "File uploaded successfully!"
            st.success(label)
            if mode == "📁 Upload a file":
                st.audio(audio_file)

            # Add spacing between input and file details
            st.markdown("<br>", unsafe_allow_html=True)

            # File details
            st.markdown("### File Details")
            st.write(f"**Filename:** {getattr(audio_file, 'name', 'live_recording.wav')}")
